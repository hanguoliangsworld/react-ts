...
const { Header } = Layout;
const popoverList = [
  { name: "个人中心", path: "/setting/base" },
  { name: "消息通知", path: "/setting/notification" },
  { name: "账号设置", path: "/setting/account" },
];
    ...
    () => (
      <div className="message-popover">
        <div className="msg-header item-block">
          ...
          <Link className="right" to="/setting/notification">
            消息接收管理
          </Link>
        </div>
        {messageList.length > 0 ? (
          <>
            ...
            <Link className="item-block ls" to="/setting/innerMessage">
              查看更多
            </Link>
          </>
        ) : (
